 ...
 * from the camera at a specified rate, sending those frames to the detector as
 * fast as it is able to process those frames.
 * <p/>
 * This camera source makes a best effort to manage processing on preview frames
 * as fast as possible, while at the same time minimizing lag. As such, frames
 * may be dropped if the detector is unable to keep up with the rate of frames
 * generated by the camera. You should use
 * {@link CameraSource.Builder#setRequestedFps(float)} to specify a frame rate
 * that works well with the capabilities of the camera hardware and the detector
 * options that you have selected. If CPU utilization is higher than you'd like,
 * then you may want to consider reducing FPS. If the camera preview or detector
 * results are too "jerky", then you may want to consider increasing FPS.
 * <p/>
...
public class CameraSource {
     ...
     * the device. See {@link Frame.Metadata#getRotation()}.
     */
     ...
     * frames, as the frames become available from the camera.
     */
     ...
     * associated byte buffer. We use byte buffers internally because this is a more
     * efficient way to call into native code later (avoids a potential copy).
     */
    ...
    public static class Builder {
         ...
         * Camera preview images will be streamed to the associated detector upon
         ...
         * value is not not available, the best matching available value is selected.
         * Default: 30.
         */
    ...
    public interface AutoFocusCallback {
         ...
         * auto-focus and autoFocus is called, onAutoFocus will be called immediately
         * with a fake value of <code>success</code> set to <code>true</code>.
         * <p/>
         * The auto-focus routine does not lock auto-exposure and auto-white balance
         * after it completes.
         *
     ...
     * {@link Camera.Parameters#FOCUS_MODE_CONTINUOUS_VIDEO} and
     * {@link Camera.Parameters#FOCUS_MODE_CONTINUOUS_PICTURE}. Applications can
     ...
     * detector. The preview frames are not displayed.
     *
    ...
    /**
     * Opens the camera and starts sending preview frames to the underlying
     * detector. The supplied surface holder is used for the preview so frames can
     ...
     * and release the resources of the underlying detector.
     */
    public void stop() {
        synchronized (mCameraLock) {
     ...
     * should have been activated previously with {@link #start()} or
     * {@link #start(SurfaceHolder)}. The camera preview is suspended while the
     * picture is being taken, but will resume once picture taking is done.
     *
     ...
     *         created. Applications should call
     *         {@link #autoFocus(AutoFocusCallback)} to start the focus if focus
     ...
     *
     * @return current flash mode. null if flash mode setting is not supported or
     *         the camera is not yet created.
     ...
     * {@link #start()} or {@link #start(SurfaceHolder)} and before {@link #stop()}
     * or {@link #release()}).
     * <p/>
     * <p>
     * Callers should check {@link #getFocusMode()} to determine if this method
     * should be called. If the camera does not support auto-focus, it is a no-op
     * and {@link AutoFocusCallback#onAutoFocus(boolean)} callback will be called
     * immediately.
     * <p/>
     * <p>
     * If the current flash mode is not {@link Camera.Parameters#FLASH_MODE_OFF},
     * flash may be fired during auto-focus, depending on the driver and camera
     * hardware.
     * <p>
     *
     ...
     * turn the camera preview back on after the picture has been taken.
     */
    ...
    private Camera createCamera() {
        ...
        Log.i(TAG, parameters.getSupportedFocusModes().toString());
            ...
            while (true) {
                synchronized (mLock) {
                    ...
                    outputFrame = new Frame.Builder()
                            .setImageData(mPendingFrameData, mPreviewSize.getWidth(), mPreviewSize.getHeight(),
                                    ImageFormat.NV21)
                            .setId(mPendingFrameId).setTimestampMillis(mPendingTimeMillis).setRotation(mRotation)
                            .build();
